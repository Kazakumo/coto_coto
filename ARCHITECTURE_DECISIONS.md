...
**このドキュメントについて**: CotoCotoプロジェクトのアーキテクチャ決定、テスト戦略、エラーハンドリング設計の詳細です。

**役割**: 設計・実装のリファレンス
**言語**: 日本語

**読み方**:
- プロジェクトで採用した技術を知りたい → 1-5. 技術決定セクション
- スケーラブルなアーキテクチャについて知りたい → 6. スケーラブルなアーキテクチャ設計
- テスト設計・テストの書き方を知りたい → 10. テスト戦略
- エラーハンドリングの分類・設計を知りたい → 11. エラーハンドリング設計
- タスク完了の要件・PR マージチェックリストを知りたい → 12. タスク完了の要件
- コーディング規則（Phoenix・Elixir）を知りたい → AGENTS.md を参照
- ワークフロー（Issue・Branch・PR）を知りたい → WORKFLOW.md を参照
